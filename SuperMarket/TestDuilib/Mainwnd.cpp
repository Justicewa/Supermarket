...
void MainWnd::Notify(TNotifyUI& msg){
	...
	if (msg.sType == _T("click")){
		...
		CComboBoxUI* pComobSelect = (CComboBoxUI*)m_PaintManager.FindControl(_T("COMOB_SELECT"));
		pComobSelect->SelectItem(0);
	}
...
void MainWnd::SelectEmployeeInfo()
{
	...
	{
		strSQL += " where gender = '";
		CDuiString strName = ((CEditUI*)m_PaintManager.FindControl(_T("usergender")))->GetText();
		if (strName.IsEmpty())
		{
			MessageBox(m_hWnd, _T("请输入查询用户的性别"), _T("Cashier"), IDOK);
			return;
		}
	...
	{
		strSQL += " where Name = '";
		CDuiString strName = ((CEditUI*)m_PaintManager.FindControl(_T("salary")))->GetText();
		if (strName.IsEmpty())
		{
			MessageBox(m_hWnd, _T("请输入查询用户的薪资"), _T("Cashier"), IDOK);
			return;
		}
